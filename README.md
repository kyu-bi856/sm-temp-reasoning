...
Tasks that are MCQs OR Timeline tasks are evaluated on accuracy using exact match, while all other tasks are evaluated on normalized cosine similarity using $sim_{norm} = \frac{1 + sim(gold, pred)}{2}$
